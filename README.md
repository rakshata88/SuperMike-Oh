...
Touch controls appear on touchscreen devices. Landscape play fills the screen with a handheld layout: **← / →** for movement, **A = Jump**, **hold B = Run**, and **C = Attack**. Each finger is tracked independently, so movement, running, jumping, and attacking can be combined. Releasing a button stops its input; focus loss, pause, orientation changes, death, and travel clear controls. **START** pauses/resumes and **SELECT** displays level information. Stand still on a marked pipe for 0.8 seconds to enter; keyboard Down/S and gamepad Down still work. There is no mobile Up or Down button. Approach the Level 3 dog house from the left and pause at its door to enter. Phones in portrait show a rotate prompt; landscape gameplay continues automatically after rotation. Sound starts muted; enable it using the music button or Settings.
...
Normal Mike and Super Mike use alternating legs and opposing shoulder/elbow/hand motion, with planted feet, knee lift, forward lean and a small body bounce. Walking has a shorter stride; running increases cadence and swing. The cycle follows actual ground distance and does not advance against walls or while a platform carries Mike. The original sprite atlas remains unchanged.

Use the in-game fullscreen button to enter fullscreen. Touch devices request landscape orientation where supported and release the lock on exit. Browsers without fullscreen support use a full-window view. Portrait phones show a rotate prompt and suspend gameplay; landscape automatically hides the prompt and continues. Returning from the background retains the normal pause screen. One viewport controller handles rotation, browser toolbar changes, safe areas and proportional canvas resizing.

A starts one jump per press, including rapid release-and-repress taps. Holding it controls height without repeated jumping. The existing coyote window is 110ms and the jump buffer is 100ms; a valid buffered tap is consumed once on landing. See [mobile polish validation](docs/mobile-polish-validation.md) for regression coverage and remaining physical-device checks.
