import {createLevel,FLOOR} from './level.js';
...
export class Game {
  ...
  reset(save=null){
    this.main=createLevel();this.cavern=createLevel(true);this.level=this.main;this.player={x:150,y:450,w:34,h:59,vx:0,vy:0,grounded:false,face:1,super:false,invuln:0,attack:0,duck:false};
    ...
    this.buffs={speed:0,blessing:0,multiplier:0,jump:0};this.jumpBuffer=0;this.coyote=0;this.transform=0;this.transition=null;this.deathTimer=0;this.camera=0;this.returnX=3870;this.particles=[];this.mode='playing';this.player.x=this.checkpoint||150;this.player.y=450;this.completed=false;this.winTime=0;this.onEvent('state');
  }
  snapshot(){return {coins:this.coins,score:this.score,paws:this.paws,lives:this.lives,elapsed:this.elapsed,kills:this.kills,checkpoint:this.checkpoint,maxX:this.maxX,collected:[...this.collected]}}
  ...
  damage(fatal=false){
    ...
    else{this.lives--;this.deathTimer=.9;p.vy=-340;p.vx=0;this.emit('death')}
    this.emit('state');
  }
  respawn(){
  ...
  travel(pipe){this.transition={timer:.6,pipe};this.player.vx=0;this.player.vy=0;this.emit('pipe')}
  pickup(it){
  ...
  step(dt,input={}){
    ...
    if(this.transition){this.transition.timer-=dt;if(this.transition.timer<=0){if(this.level.cave){this.level=this.main;p.x=this.returnX;p.y=430;this.emit('toast','Back in the sunshine.')}else{this.returnX=this.transition.pipe.x+120;this.level=this.cavern;p.x=220;p.y=450;this.emit('toast','THE WHISKER CAVERN · Find the royal paw')}this.camera=Math.max(0,p.x-250);p.invuln=1.5;p.vx=0;p.vy=0;this.transition=null;this.emit('state')}return}
    ...
    if(this.jumpBuffer>0&&this.coyote>0){p.vy=-(p.super?760:700)*(this.buffs.jump>0?1.2:1);p.grounded=false;this.coyote=0;this.jumpBuffer=0;this.emit('jump')}
    if(!input.jump&&p.vy<-240)p.vy+=2100*dt;
    if(input.attackPressed&&p.super&&p.attack<=0){p.attack=.32;this.emit('punch')}
    ...
    for(const s of platforms)if(overlap(p,s)){
      ...
      else if(p.vy<0){p.y=s.y+s.h;p.vy=0;if(s.type==='breakable'&&p.super){s.broken=true;this.burst(s.x+s.w/2,s.y,'#bf976b',20);this.score+=100}}
    }
    ...
    for(const e of this.level.enemies){
      ...
      if(e.type==='pipe'){e.y=e.baseY+Math.max(0,Math.sin(e.timer*1.4))*90;e.hidden=e.y>e.baseY+30;if(e.hidden)continue}
        ...
        let speed=e.type==='fast'?155:e.type==='chonky'?35:55;
        if(e.type==='sleepy'&&Math.abs(p.x-e.x)>220)speed=0;
        if(e.type==='fast'&&Math.abs(p.x-e.x)<360)e.vx=Math.sign(p.x-e.x)*speed;
        ...
        e.vy+=1350*dt;e.y+=e.vy*dt;if(e.y+e.h>=FLOOR){e.y=FLOOR-e.h;e.vy=0}
      }
      const punch={x:p.face>0?p.x+p.w:p.x-55,y:p.y+10,w:55,h:55};
      if(p.attack>0&&overlap(punch,e)){this.defeat(e);continue}
      if(overlap(p,e)){
        if(this.buffs.blessing>0){this.defeat(e);continue}
        if(p.vy>0&&previousBottom<=e.y+17&&e.type!=='pipe'){e.hp-=p.super?3:1;p.vy=-430;p.y=e.y-p.h;if(e.hp<=0)this.defeat(e);else{this.emit('stomp');this.burst(e.x+20,e.y,'#d0e5e4',5)}}else this.damage();
      }
    }
    for(const h of this.level.hazards)if(overlap(p,h))this.damage();
    ...
    if(!this.level.cave){this.maxX=Math.max(this.maxX,p.x);if(!this.checkpoint&&p.x>=this.main.checkpoint){this.checkpoint=this.main.checkpoint;this.burst(p.x,p.y,'#f4d17b',25);this.emit('checkpoint');this.emit('toast','CHECKPOINT! · Your courage is saved.');this.emit('save')}
      if(p.x>=this.main.goal){this.completed=true;this.mode='won';this.winTime=0;this.bonus=Math.max(0,Math.round(600-this.elapsed))*10+this.lives*500;this.score+=this.bonus;this.emit('win');this.emit('state')}
    }
