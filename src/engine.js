...
export class Game {
  ...
  damage(fatal=false){
    ...
    if((p.form==='fire'||p.form==='thunder')&&!fatal){const poweredForm=p.form;p.form='super';if(poweredForm==='thunder'){p.thunderTime=0;p.attack=0}p.invuln=2;p.vy=-280;this.emit('hurt');this.emit('toast',(poweredForm==='thunder'?'Thunder Mike':'Fire Mike')+' → Super Mike. Keep going!')}
  ...
  step(dt,input={}){
    ...
    if(this.transition){this.transition.timer-=dt;if(this.transition.timer<=0){if(this.level.cave){this.level=this.main;p.x=this.returnX;p.y=430;this.emit('toast',this.levelId===4?'Back in the castle halls.':'Back in the sunshine.')}else{this.returnX=this.transition.pipe.x+120;this.level=this.cavern;p.x=220;p.y=450;this.emit('toast',this.levelId===4?'ROYAL ARCHIVES — KEEP OUT · A secret treasury!':this.levelId===3?'SECRET DOG HOUSE · DOGS ONLY (Mike disagrees!)':this.levelId===2?'GOLDEN WHISKER VAULT · A royal bonus room':'THE WHISKER CAVERN · Find the royal paw')}this.camera=Math.max(0,p.x-250);p.invuln=1.5;p.vx=0;p.vy=0;this.transition=null;this.emit('state')}return}
    ...
    if(this.levelId===4)updateCastle(this,dt);
    ...
    p.x+=p.vx*dt;
    for(const s of platforms)if(overlap(p,s)){if(s.skin==='shelf'&&s.dx&&Math.abs(p.vx)<1)p.x=s.dx>0?s.x+s.w:s.x-p.w;else if(p.vx>0)p.x=s.x-p.w;else if(p.vx<0)p.x=s.x+s.w;p.vx=0}
    ...
    for(const pipe of this.level.pipes)if(pipe.door&&!pipe.archive&&pipe.secret&&!pipe.revealed&&((p.x>pipe.x-100&&p.x<pipe.x+20)||this.level.platforms.some(s=>s.id&&s.used&&Math.abs(s.x-pipe.x)<150))){pipe.revealed=true;this.emit('toast','A secret door! Stand still by the dog house.')}
    ...
    for(const e of this.level.enemies){
      ...
      if(e.faction==='shadow'){updateShadowEnemy(this,e,dt,platforms);if(e.hidden||e.harmless||['defeated','intro','stunned','sleep'].includes(e.state))continue}
      ...
      const attackActive=p.attack>0&&(!p.attackMove?.electric||p.attack>p.attackMove.duration-p.attackMove.active);
      if(attackActive&&!p.attackMove?.projectile&&overlap(punch,e)&&!this.attackHits?.has(e)){this.attackHits?.add(e);if(e.faction==='shadow'){if(!hitShadow(this,e,p.attackMove.electric?'electric':'punch')&&overlap(p,e))this.damage();continue}if(e.faction==='dog'){if(!hitDog(this,e)&&overlap(p,e))this.damage();continue}e.hp-=e.type==='guard'?Math.min(2,p.attackMove.damage):p.attackMove.damage;if(e.hp<=0)this.defeat(e);else{e.stun=.4;this.burst(e.x+20,e.y,'#ffe18b',6);this.emit('stomp')}continue}
      if(overlap(p,e)){
        if(this.buffs.blessing>0){if(e.faction==='shadow')hitShadow(this,e,'stomp');else if(e.faction==='dog')hitDog(this,e,'stomp');else this.defeat(e);continue}
        if(p.vy>0&&previousBottom<=e.y+17&&e.type!=='pipe'){if(e.faction==='shadow'){const hit=hitShadow(this,e,'stomp');p.vy=-430;p.y=e.y-p.h;if(!hit&&e.invuln<=0)this.damage();continue}if(e.faction==='dog'){const hit=hitDog(this,e,'stomp');p.vy=-430;p.y=e.y-p.h;if(!hit)this.damage();continue}e.hp-=e.type==='guard'?1:p.super?3:1;p.vy=-430;p.y=e.y-p.h;if(e.hp<=0)this.defeat(e);else{this.emit('stomp');this.burst(e.x+20,e.y,'#d0e5e4',5)}}else if(e.faction==='dog'||e.faction==='shadow'||!(p.attack>0&&this.attackHits?.has(e)))this.damage();
      }
    }
    ...
    if(!this.level.cave){this.maxX=Math.max(this.maxX,p.x);for(const cp of this.main.checkpoints||[this.main.checkpoint])if((!this.main.checkpoints||!this.deathTimer)&&cp>this.checkpoint&&p.x>=cp){this.checkpoint=cp;this.burst(p.x,p.y,'#f4d17b',25);this.emit('checkpoint');this.emit('toast',this.main.checkpoints&&cp===this.main.checkpoints.at(-1)?'FINAL CHECKPOINT!':'CHECKPOINT! · Your courage is saved.');this.emit('save')}
      if(this.levelId!==4&&!this.deathTimer&&p.x>=this.main.goal&&(!this.main.boss||this.main.gateOpen)){this.finishLevel()}
    }
