...
const game=new Game((type,message)=>{
  if(type==='inputreset')clearInput();
    ...
    else{data.save=null;persist();winTimer=setTimeout(showVictory,10000)}
  }
...
function startPlay(saved=false,carry=false){$('#game-shell').classList.add('is-session');lastAutoSave=0;clearTimeout(winTimer);if(carry)game.nextLevel();else game.reset(saved?data.save:null,saved?(data.save?.levelId||1):selectedLevel);selectedLevel=game.levelId;game.mode='playing';screen.innerHTML='';hud.hidden=false;$('#game-bottom').hidden=true;$('#touch-controls').hidden=false;clearInput();canvas.focus({preventScroll:true});audio.unlock();updateHUD();toast(saved?'Welcome back, Mike. Your checkpoint is ready.':game.levelId===3?'LEVEL 3 — THE DOG GUARD TERRITORY · GO!':game.levelId===2?'LEVEL 2 — THE CAT KINGDOM · GO!':'GREEN PAW MEADOWS · Prince Xiaboo needs you!');data.save=game.snapshot();persist()}
...
function togglePause(){if(game.mode==='playing')pause();else if(game.mode==='paused')resume();else if(game.mode==='levelintro')(selectedLevel===3?actions.territoryGo:actions.kingdomGo)();else screen.querySelector('.primary-button')?.click()}
...
function levels(){
  ...
  const choices=[[1,'The Beginning','start'],[2,'The Cat Kingdom','kingdom'],[3,'The Dog Guard Territory','territory']].map(([id,title,action])=>{
