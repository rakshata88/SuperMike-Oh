...
const game=new Game((type,message)=>{
  if(type==='inputreset')clearInput();
    ...
    else if(game.levelId===4){data.save={levelId:5,coins:game.coins,score:game.score,lives:game.lives,kills:game.kills,collected:[]};persist();modal('<span class="eyebrow">LEVEL 4 COMPLETE!</span><h2>Lord Whiskeron escaped!</h2><p>From the balcony, Mike sees a large convoy leaving the kingdom.<br>Prince Xiaboo is being taken toward a new region.</p><p><strong>MIKE: “I’m not letting him get away!”</strong></p>');winTimer=setTimeout(()=>escapeIntro(true),3200)}
...
function updateHUD(){if(!window.__mikeReady)return;const stamp=[game.lives,game.coins,game.paws,game.score,game.player.super,game.player.form,game.level.cave,game.levelId,game.level.roomId,game.chaseRegion].join();if(stamp===hudStamp)return;hudStamp=stamp;$('#hud-lives').textContent='♥ '.repeat(Math.max(0,Math.min(5,game.lives)))+(game.lives>5?`+${game.lives-5}`:'');$('#hud-coins').textContent=String(game.coins).padStart(2,'0');$('#hud-paws').textContent=`${game.paws}/3`;$('#hud-score').textContent=String(game.score).padStart(6,'0');$('#hud-power').textContent=game.player.form==='dash'?'» DASH MIKE':game.player.form==='thunder'?'ϟ THUNDER MIKE':game.player.form==='fire'?'✦ FIRE MIKE':game.player.super?'✦ SUPER MIKE':'NORMAL MIKE';$('#hud-zone').textContent=game.level.cave?(game.levelId===5?(game.level.roomId==='cargo'?'ROYAL CARGO CAR':'FOX TREASURE DEN'):game.levelId===4?'ROYAL ARCHIVES':game.levelId===3?'SECRET DOG HOUSE':game.levelId===2?'GOLDEN WHISKER VAULT':'THE WHISKER CAVERN'):game.levelId===5?escapeSection(game.player.x).name.toUpperCase():game.main.title.toUpperCase()}
...
function startPlay(saved=false,carry=false){$('#game-shell').classList.add('is-session');lastAutoSave=0;clearTimeout(winTimer);if(carry)game.nextLevel();else game.reset(saved?data.save:null,saved?(data.save?.levelId||1):selectedLevel);selectedLevel=game.levelId;game.mode='playing';screen.innerHTML='';hud.hidden=false;$('#game-bottom').hidden=true;$('#touch-controls').hidden=false;clearInput();canvas.focus({preventScroll:true});audio.unlock();updateHUD();toast(saved?'Welcome back, Mike. Your checkpoint is ready.':game.levelId===5?'LEVEL 5 — THE GREAT ESCAPE · CHASE LORD WHISKERON!':game.levelId===4?'LEVEL 4 — THE SHADOW CASTLE · Find Prince Xiaboo!':game.levelId===3?'LEVEL 3 — THE DOG GUARD TERRITORY · GO!':game.levelId===2?'LEVEL 2 — THE CAT KINGDOM · GO!':'GREEN PAW MEADOWS · Prince Xiaboo needs you!');data.save=game.snapshot();persist()}
...
function escapeIntro(carry=false){
  if(!canSelectLevel(data,5))return;selectedLevel=5;game.mode='levelintro';hud.hidden=true;
  $('#game-shell').classList.add('is-session');$('#game-bottom').hidden=true;
  modal('<span class="eyebrow">LEVEL 5</span><h2>THE GREAT ESCAPE</h2><p><strong>CHASE LORD WHISKERON!</strong><br>Through the forest, across the river, aboard a speeding train, and into the mountains...</p><p>Hold B to run. A jumps. C attacks.<br>Dash Mike: hold B, reach full speed, then press C.<br>Watch for warnings. Pause on green pipes to explore.</p><div class="modal-actions"><button class="primary-button" data-action="escapeGo">GO!</button></div>');
  actions.escapeGo=()=>{clearTimeout(winTimer);if(game.mode==='levelintro')startPlay(false,carry)};winTimer=setTimeout(actions.escapeGo,8000);
}
function togglePause(){if(game.mode==='playing')pause();else if(game.mode==='paused')resume();else if(game.mode==='levelintro')(selectedLevel===5?actions.escapeGo:selectedLevel===4?actions.castleGo:selectedLevel===3?actions.territoryGo:actions.kingdomGo)();else screen.querySelector('.primary-button')?.click()}
...
function levels(){
  ...
  const choices=[[1,'The Beginning','start'],[2,'The Cat Kingdom','kingdom'],[3,'The Dog Guard Territory','territory'],[4,'The Shadow Castle','castle'],[5,'The Great Escape','escape']].map(([id,title,action])=>{
