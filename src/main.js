...
const game=new Game((type,message)=>{
  if(type==='inputreset')clearInput();
    ...
    else{data.save=null;persist();toast('“MEOW!” · Prince Xiaboo is close...');audio.sfx('meow');winTimer=setTimeout(showVictory,3200)}
  }
...
function updateHUD(){if(!window.__mikeReady)return;const stamp=[game.lives,game.coins,game.paws,game.score,game.player.super,game.level.cave,game.levelId].join();if(stamp===hudStamp)return;hudStamp=stamp;$('#hud-lives').textContent='♥ '.repeat(Math.max(0,Math.min(5,game.lives)))+(game.lives>5?`+${game.lives-5}`:'');$('#hud-coins').textContent=String(game.coins).padStart(2,'0');$('#hud-paws').textContent=`${game.paws}/3`;$('#hud-score').textContent=String(game.score).padStart(6,'0');$('#hud-power').textContent=game.player.super?'✦ SUPER MIKE':'NORMAL MIKE';$('#hud-zone').textContent=game.level.cave?(game.levelId===2?'GOLDEN WHISKER VAULT':'THE WHISKER CAVERN'):game.main.title.toUpperCase()}
